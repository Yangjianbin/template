...
if (window.addEventListener) {
	window.addEventListener('DOMContentLoaded', function() {
		...
		slider('.slide', '.slide-con', '.slide-item', '.tab-nav', 'a', 'tab-nav-cur', 300, 8000);
		...
		function Slider(slide, slideCon, slideItem, nav, navItem, navCur, delay, autoTime) {
			...
			function autoPlay() {
				if (autoTime < 3 * delay || isBegin || isMove || !isEnd) {
					return;
				}
			...
			this.move = function(e) {
				...
				if (dir.horizontal) {
					...
					if ((cur === 0 && x > 0) || (cur === length - 1 && x < 0)) {
						Slider.transform(slideCon, ((x / 2 / width - cur) * 100 / length) + '%');
					} else {
...
function click_a(divDisplay)
{
	...
	if(obj.style.display != "block")
	{
		...
		category_s.className="spanclick"
		last_div = obj;
